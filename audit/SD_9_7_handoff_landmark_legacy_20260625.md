# HISTORICAL SD-9.7 Landmark-Era Handoff (through 2026-06-25)

This snapshot contains superseded landmark queues, launch commands, and live-state notes. Retain it for
provenance only. Do not execute its standing loop. Current operating instructions are in
...
## 8. CANONICAL SCHEDULER (2026-06-25) — use `run_sd_grid.sh`, retire the legacy launchers for NEW work

The ad-hoc `run_sd9_6_*` / `run_sd9_7_*` launchers had NO cross-launcher lock → concurrent/overlapping
launches could duplicate compute (the 2026-06 hazard). Replaced by `scripts/run_sd_grid.sh`
(+ `scripts/sd_grid_status.py`), which is **duplication-proof + resumable + OOM-mapping**:
- **Mutual exclusion:** every (family,backend,L,variant,seed) cell is assigned to ONE host in the manifest
  (blue ≤4096, lizmark ≥8192); intra-host an atomic `mkdir` lock + triple-skip (`.done` marker /
  metadata-complete via `sd_grid_status.py` / **live-process** ps-check) means no cell ever runs twice, even
  alongside a still-running legacy worker.
- **Resumable:** complete cells leave `markers/*.done`; a failed/killed cell releases its lock → next run
  retries it. Re-run the driver any number of times; it only advances missing cells.
- **OOM map:** OOM → `out/paper_mechanisms/sd_grid/oom_registry.tsv` (the VRAM-ceiling grid) + `.oom`
  record-and-skip marker; the queue keeps going (use this to show max-VRAM support across the grid).
- **Defaults:** `SEEDS="0 1 2"` (3 seeds). Top up best variants later with `SEEDS="3 4"`.
- **Grid:** set {b0,b25,b50,b62,b75,b100} × L{512,2048,4096,8192,16384,32768} + landmark token baselines.

**Run (always DRY_RUN=1 first to see the plan):**
```
HOST_TAG=blue    DRY_RUN=1 bash scripts/run_sd_grid.sh        # inspect plan
...
```
**Cutover discipline:** do NOT start the grid concurrently with a legacy queue on the same host (legacy
ignores the lock) and only onto FREE GPUs (L=16384 needs ~37GB, L=32768 more — expect OOM rows, which is the
point). Let legacy drain, then the grid is sole scheduler. One grid driver per host (it spawns 2 GPU workers).

**Reporting:** `python3 scripts/sd_grid_status.py out/paper_mechanisms` (backend-aware; dense vs landmark
token no longer conflated) → aggregate mean PPL/VRAM per (family,backend,L,variant). Corrected headline:
optimal blur ≈ **25% for L≤4096**, **~62% at 8192** (b25 wing flipped the earlier "mixed50 wins").

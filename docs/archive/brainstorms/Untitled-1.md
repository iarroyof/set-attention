...
$\PhiS_i = k_\Delta(S_i, \cdot) = [K_\Delta[i,1], K_\Delta[i,2], \ldots, K_\Delta[i,m]]^T \in \mathbb{R}^{m}$
...
$h_t = \sum_{h=1}^H w_h \cdot \text{Route}_h(Z_{sh})$
...
When using kernel-row features $\PhiS_i = K_\Delta[i, :] \in \mathbb{R}^m$:

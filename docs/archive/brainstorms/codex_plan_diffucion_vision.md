gotcha — here’s a single **merged markdown frame** that keeps the **correct project structure + code snippets** from your latest version and **restores the missing details** (theory, learning behavior, guarantees, acceptance criteria, profiling targets, ablations, etc.) from the earlier write-up.

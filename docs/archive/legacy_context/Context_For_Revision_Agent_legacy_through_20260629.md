...
Set-dictionary branch overlay (current as of 2026-06-29):

- For `set-dictionary/anchor-span`, read `docs/set_dictionary_dev_agent_prompt.md` after this file.
- Current experiment scope is exact dense set dictionary versus exact dense token, defined by
  `docs/sd_dense_matched_comparison_plan.md`.
- Live state is `audit/phase_sd_status.md`; operating instructions are `audit/SD_9_7_handoff.md`.
- The older locked plan remains the architecture/history source, but its landmark launch policy is
  superseded for this branch. Coverage-scaled landmark rows are historical reference only and are not
  active linear-efficiency evidence.
- blue-demon owns current `L<=1024` dense cells; lizmark (`192.168.241.205`) owns current `L>=2048`
  dense cells. Both host copies may therefore contain authoritative experiment artifacts for this branch.
...
- Set-dictionary dense-grid large-memory server: `lizmark:~/set-attention`
- Local paper/workspace mirror may exist at:
  `/mnt/d/UserFolders/Documents/GitHub/set-attention`
- Treat the host assigned by the current set-dictionary manifest as the source of truth for that cell;
  blue-demon is the default source for the original Phase A corpus.

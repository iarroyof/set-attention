...
\begin{itemize}
    ...
    \item $V \in \mathbb{N}$ denote the vocabulary size.
\end{itemize}
...
\end{equation}
where $L_{\max}$ is the maximum sequence length. Positional embeddings are learned absolute embeddings.

\paragraph{Input token sequence.}
An input token sequence is
\begin{equation}
\label{eq:input-sequence}
\mathbf{x}=(x_1,\dots,x_L)\in\{1,\dots,V\}^L.
\end{equation}
...
\operatorname{TokenMLP}(\mathbf{x})=\mathbf{W}_2\,\operatorname{GELU}(\mathbf{W}_1\mathbf{x}),
\end{equation}
with
\begin{equation}
...
S_m=\{a_m,a_m+1,\dots,a_m+w-1\}\subseteq\{1,\dots,L\}.
\end{equation}
Thus the bank is the finite set family
\begin{equation}
\label{eq:set-family}
\mathcal{S}=\{S_1,\dots,S_M\}\subseteq 2^{\{1,\dots,L\}}.
\end{equation}
...
For each token position $t\in V$, define its \emph{candidate family}
\begin{equation}
...
=\frac{1}{w}\sum_{t\in S_m}\mathbf X_{t,:}
\in\mathbb R^D.
\end{equation}
...
Within each block, set attention is standard multi-head self-attention with $H_a$ heads. For each attention head $r\in\{1,\dots,H_a\}$,
\begin{align}
\label{eq:qkv-projections}
\mathbf Q^{(r)} &= \mathbf S\mathbf W_Q^{(r)},
&\mathbf W_Q^{(r)}\in\mathbb R^{D\times d_a},\\
\mathbf K^{(r)} &= \mathbf S\mathbf W_K^{(r)},
&\mathbf W_K^{(r)}\in\mathbb R^{D\times d_a},\\
\mathbf V^{(r)} &= \mathbf S\mathbf W_V^{(r)},
&\mathbf W_V^{(r)}\in\mathbb R^{D\times d_a}.
\end{align}
The masked score matrix is
\begin{equation}
\label{eq:attention-scores}
\widetilde{\mathbf A}^{(r)}
=
\frac{\mathbf Q^{(r)}(\mathbf K^{(r)})^\top}{\sqrt{d_a}}
+ (\mathbf 1-\mathbf C)(-\infty),
\end{equation}
and the corresponding attention output is
\begin{equation}
\label{eq:attention-output}
\mathbf O^{(r)}=\operatorname{softmax}(\widetilde{\mathbf A}^{(r)})\mathbf V^{(r)}.
\end{equation}
...
\qquad m\in\{1,\dots,M\}.
\end{equation}
The exact parametrization of these scores depends on the learned router, but the theoretical analysis below only requires the induced masked softmax distributions.
...
\end{cases}
\end{equation}
In the main experiments, $k=\infty$, so only topology-induced masking is active.
...
\mathbb Q_t^{(u)}\in\mathcal P(\mathcal C_t)
\end{equation}
with mass function
\begin{equation}
...
\bar{\mathbb Q}_t\in\mathcal P(\mathcal C_t),
\qquad
...
\mathcal E_{\mathrm{agg}}
=
\frac{1}{L}\sum_{t=1}^L \mathcal E(\bar{\mathbb Q}_t).
\end{equation}
...
\mathcal E_{\mathrm{agg,norm}}(t)
=
\frac{\mathcal E(\bar{\mathbb Q}_t)}{\log C_t},
\qquad C_t\ge 2,
\end{equation}
and the mean normalized aggregated entropy by
\begin{equation}
\label{eq:normalized-agg-entropy}
\mathcal E_{\mathrm{agg,norm}}
=
\frac{1}{|\{t:C_t\ge 2\}|}
\sum_{t:C_t\ge 2}
\frac{\mathcal E(\bar{\mathbb Q}_t)}{\log C_t}.
...
\max_{m\in\{1,\dots,M\}}
\sum_{u=1}^{H_r}
\mathbbm{1}_{\left[\arg\max_j \pi_{t,j}^{(u)}=m\right]}
...
Fix a token position $t \in \{1,\dots,L\}$ and a router head $r \in \{1,\dots,H_r\}$. Let
\[
\mathbb{Q}^{(r)}_{t}
\]
be the per-head routing probability measure on the candidate fiber $\mathcal{C}_{t}$, with mass function
\[
\pi^{(r)}_{t,m} = \mathbb{Q}^{(r)}_{t}(\{m\}), \qquad m \in \{1,\dots,M\},
\]
...
\begin{enumerate}
    ...
    H\!\left(\mathbb{Q}^{(r)}_{t}\right)
    :=
    - \sum_{m=1}^{M} \pi^{(r)}_{t,m}\log \pi^{(r)}_{t,m}
    \leq
    \log \left| \operatorname{supp}\!\left(\mathbb{Q}^{(r)}_{t}\right)\right|.
    \end{equation}

    \item Equality in \eqref{eq:per-head-entropy-bound-thm} holds if and only if $\mathbb{Q}^{(r)}_{t}$ is uniform on its support.

    \item If the masked routing logits $\tilde g^{(r)}_{t,m}$ are fixed and the support does not change with $\tau_r$, then
    \begin{equation}
    \label{eq:entropy-monotone-temp}
    \tau_r \longmapsto H\!\left(\mathbb{Q}^{(r)}_{t}\right)
    \end{equation}
    is nondecreasing.
    ...
    \lim_{\tau_r \to 0^+} H\!\left(\mathbb{Q}^{(r)}_{t}\right)=0
    \end{equation}
    ...
    \lim_{\tau_r\to\infty} H\!\left(\mathbb{Q}^{(r)}_{t}\right)
    =
    \log \left|\operatorname{supp}\!\left(\mathbb{Q}^{(r)}_{t}\right)\right|.
    \end{equation}
\end{enumerate}
...
Let
\[
S_{t}^{(r)} := \operatorname{supp}\!\left(\mathbb{Q}^{(r)}_{t}\right)
\subseteq \mathcal{C}_{t}.
\]
Since $\mathbb{Q}^{(r)}_{t}$ is a probability measure on the finite set $S_{t}^{(r)}$, its Shannon entropy is maximized by the uniform distribution on that support. This is the classical finite-support entropy maximization principle. Therefore
\[
H\!\left(\mathbb{Q}^{(r)}_{t}\right)\le \log |S_{t}^{(r)}|,
\]
which proves \eqref{eq:per-head-entropy-bound-thm}.

\paragraph{Step 2: Characterization of equality.}
The Shannon entropy is strictly concave on the probability simplex. Therefore the unique maximizer over distributions supported on $S_{t}^{(r)}$ is the uniform distribution:
\[
\pi^{(r)}_{t,m} = \frac{1}{|S_{t}^{(r)}|} \qquad \text{for all } m\in S_{t}^{(r)}.
\]
Hence equality holds if and only if $\mathbb{Q}^{(r)}_{t}$ is uniform on its support.

\paragraph{Step 3: Softmax form.}
On the support $S_{t}^{(r)}$, the routing probabilities are
\[
\pi^{(r)}_{t,m}(\tau_r)
=
\frac{\exp(\tilde g^{(r)}_{t,m}/\tau_r)}
{\sum_{j\in S_{t}^{(r)}} \exp(\tilde g^{(r)}_{t,j}/\tau_r)}.
\]
...
-
\frac{1}{\tau_r}
\mathbb{E}_{\mathbb{Q}^{(r)}_{t}}[\tilde g^{(r)}_{t,\cdot}].
\label{eq:entropy-partition-form}
\end{align}

\paragraph{Step 4: Derivative of the partition function.}
Differentiate $Z_{t}^{(r)}(\tau_r)$:
\[
\frac{d}{d\tau_r} Z_{t}^{(r)}(\tau_r)
=
-\frac{1}{\tau_r^2}
\sum_{j\in S_{t}^{(r)}} \tilde g^{(r)}_{t,j}
\exp(\tilde g^{(r)}_{t,j}/\tau_r).
\]
Hence
\[
\frac{d}{d\tau_r}\log Z_{t}^{(r)}(\tau_r)
=
-\frac{1}{\tau_r^2}
\mathbb{E}_{\mathbb{Q}^{(r)}_{t}}[\tilde g^{(r)}_{t,\cdot}].
\]
...
=
-\frac{1}{\tau_r^2}
\operatorname{Var}_{\mathbb{Q}^{(r)}_{t}}(\tilde g^{(r)}_{t,\cdot}).
\]
...
\[
\pi^{(r)}_{t,m}(\tau_r)\to \delta_{m,m^\star}
...
\[
\exp(\tilde g^{(r)}_{t,m}/\tau_r)\to 1
\quad\text{for } m\in S_{t}^{(r)}.
\]
Hence the routing measure converges to the uniform law on $S_{t}^{(r)}$, whose entropy is $\log |S_{t}^{(r)}|$. This proves \eqref{eq:entropy-high-temp-limit}.
\end{proof}

\begin{corollary}[Normalized per-head routing entropy]
\label{cor:normalized-head-entropy}
If $\left|\operatorname{supp}\!\left(\mathbb{Q}^{(r)}_{t}\right)\right|\ge 2$, then the normalized per-head routing entropy
\begin{equation}
\label{eq:normalized-head-entropy}
\overline H^{(r)}_{t}
:=
\frac{H\!\left(\mathbb{Q}^{(r)}_{t}\right)}
{\log \left|\operatorname{supp}\!\left(\mathbb{Q}^{(r)}_{t}\right)\right|}
\in [0,1].
\end{equation}
Moreover, $\overline H^{(r)}_{t}=0$ for deterministic routing and $\overline H^{(r)}_{t}=1$ for uniform routing on support.
\end{corollary}
...
\bar{\pi}_{t,m}
=
\frac{1}{H_r}\sum_{r=1}^{H_r} \pi^{(r)}_{t,m}
\]
...
H(\bar{\mathbb{Q}}_{t})
\ge
\frac{1}{H_r}\sum_{r=1}^{H_r}
H\!\left(\mathbb{Q}^{(r)}_{t}\right).
\end{equation}
Consequently, averaging over tokens,
\begin{equation}
\label{eq:agg-entropy-jensen-mean}
\mathcal{H}_{\mathrm{agg}}
\ge
\mathcal{H}_{\mathrm{head}},
\end{equation}
where
\[
\mathcal{H}_{\mathrm{agg}}
...
\mathcal{H}_{\mathrm{head}}
=
\frac{1}{H_r L}\sum_{r=1}^{H_r}\sum_{t=1}^{L} H\!\left(\mathbb{Q}^{(r)}_{t}\right).
\]
...
\[
H\!\left(\frac{1}{H_r}\sum_{r=1}^{H_r}\mathbb{Q}^{(r)}_{t}\right)
\ge
\frac{1}{H_r}\sum_{r=1}^{H_r} H\!\left(\mathbb{Q}^{(r)}_{t}\right).
\]
But by definition,
\[
\frac{1}{H_r}\sum_{r=1}^{H_r}\mathbb{Q}^{(r)}_{t}
=
\bar{\mathbb{Q}}_{t}.
\]
...
The quantity
\[
\mathcal{H}_{\mathrm{agg}}-\mathcal{H}_{\mathrm{head}}
\]
...
Suppose top-$k$ masking is applied before softmax, with $k\in \mathbb{N}$. Then for every token $t$ and head $r$,
\begin{equation}
\label{eq:per-head-support-topk}
\left| \operatorname{supp}\!\left(\mathbb{Q}^{(r)}_{t}\right)\right|
\le
\min\{k, C_{t}\},
\end{equation}
...
\subseteq
\bigcup_{r=1}^{H_r}
\operatorname{supp}\!\left(\mathbb{Q}^{(r)}_{t}\right).
\]
...
\begin{enumerate}
    \item For every head $r$ and every token $t$, the routing law is deterministic:
    \[
    \mathbb{Q}^{(r)}_{t} = \delta_{m^\star(t)}
    \]
    where $m^\star(t)$ is the unique element of $\mathcal{C}_{t}$.

    \item For every head $r$ and every token $t$,
    \begin{equation}
    \label{eq:degenerate-entropy-zero}
    H\!\left(\mathbb{Q}^{(r)}_{t}\right)=0.
    \end{equation}
...
\]
After candidate masking, all logits except the one corresponding to $m^\star(t)$ are equal to $-\infty$. Therefore for every head $r$,
\[
\pi^{(r)}_{t,m^\star(t)}
=
\frac{\exp(g^{(r)}_{t,m^\star(t)}/\tau_r)}
{\exp(g^{(r)}_{t,m^\star(t)}/\tau_r)}
=
1,
\]
...
For a token $t$, define the routing tensor
\[
\boldsymbol{\Pi}_{t} \in \mathbb{R}^{H_r \times M},
\qquad
(\boldsymbol{\Pi}_{t})_{r,m} = \pi^{(r)}_{t,m}.
\]
Each row is a probability vector:
\[
\sum_{m=1}^{M} (\boldsymbol{\Pi}_{t})_{r,m}=1,
\qquad
(\boldsymbol{\Pi}_{t})_{r,m}\ge 0.
\]
\end{definition}
...
\end{equation}
for some probability vector $\boldsymbol{\pi}_{t}\in \Delta^{M-1}$.
\end{proposition}
...
\[
\pi^{(1)}_{t,\cdot} = \delta_a,
\qquad
\pi^{(2)}_{t,\cdot} = \delta_b.
\]
...
There exists a constant $L_{\mathrm{out}}>0$ such that, in the neighborhood of interest,
\begin{equation}
...
\Delta \mathcal{L}
=
\mathcal{L}\bigl(\mathcal{R}\circ \Phi_{\mathrm{set}}\circ \mathcal{P}(\mathbf{X})\bigr)
-
\mathcal{L}\bigl(\mathcal{R}_{\mathrm{ref}}\circ \Phi_{\mathrm{ref}}\circ \mathcal{P}_{\mathrm{ref}}(\mathbf{X})\bigr).
\]
\end{theorem}
...
1 \le n_{\mathrm{eff}}^{(m)} \le w,
\qquad
\frac{1}{w}\le \rho_{\mathrm{eff}}^{(m)}\le 1.
\end{equation}
\end{proposition}
...
Thus
\[
1 \le w \sum_{t\in S_m}\omega_{t,m}^2,
\qquad\text{so}\qquad
\sum_{t\in S_m}\omega_{t,m}^2 \ge \frac{1}{w}.
\]
...
1 \le n_{\mathrm{eff}}^{(m)} \le w.
\]
Dividing by $w$ proves the normalized bound.
\end{proof}
...
Let $J_{\mathcal{P}}$ be the Jacobian of the pooling map at the current pre-pooling state, and let $J_{\Phi}$ be the Jacobian of the set-processing map at the current set state. Assume that their smallest eigenvalues are:
\[
...
0<\eta_{\min}\le \overline{\mathcal{H}}_{\mathrm{agg}} \le \eta_{\max}<1.
\end{equation}

#!/usr/bin/env bash
# SD-8 capacity pivot: anchor_span, all_past candidate fiber, dense router score path, CE only.
set -euo pipefail
...
if [[ "${SMOKE}" == "1" ]]; then
  ...
  LOG_ROOT="${LOG_ROOT:-logs/sd8_all_past_dense_routerdense_smoke}"
  OUT_ROOT="${OUT_ROOT:-out/paper_mechanisms/sd8_all_past_dense_routerdense_smoke}"
  GROUP_PREFIX="${GROUP_PREFIX:-sd8_all_past_dense_routerdense_smoke}"
  SEQ_LEN=64
  BATCH=2
  EPOCHS=1
  D_MODEL=64
  ...
  LOG_ROOT="${LOG_ROOT:-logs/sd8_all_past_dense_routerdense}"
  OUT_ROOT="${OUT_ROOT:-out/paper_mechanisms/sd8_all_past_dense_routerdense}"
  GROUP_PREFIX="${GROUP_PREFIX:-sd8_all_past_dense_routerdense}"
  SEQ_LEN=512
  BATCH=16
  EPOCHS=10
  D_MODEL=384
...
Path(f"audit/SD_8_all_past_dense_routerdense_prelaunch{suffix}.json").write_text(
...
if [[ "${SMOKE}" == "1" ]]; then
  ...
  PID1="$!"
  echo "SD-8 all_past dense-router workers launched: GPU0 PID=${PID0}, GPU1 PID=${PID1}"
  wait "${PID0}"
  wait "${PID1}"
fi
echo "=== SD-8 all_past dense-router complete ==="

#!/usr/bin/env bash
# A7 empty_only calibration sweep: set-dense exact compression path.
set -euo pipefail

cd ~/set-attention

LOG_ROOT="${LOG_ROOT:-logs/a7_empty_only_calibration}"
...
baseline_rows = []
summary = Path("out/paper_integrated_evidence/tables/a2_lrnorm_headline_all_runs.tsv")
if summary.exists():
        ...
        for row in csv.DictReader(fh, delimiter="\t"):
            if (
                ...
                baseline_rows.append({
                    ...
                    "final_val_ppl": row.get("final_val_ppl"),
                })

Path("audit/A7_empty_only_calibration_prelaunch.json").write_text(json.dumps({
    "branch": run(["git", "branch", "--show-current"]),
    "head": run(["git", "rev-parse", "HEAD"]),
    "status_short": run(["git", "status", "--short"]),
    "baseline_summary": str(summary),
    "baseline_match_contract": {
    ...
    "baseline_rows": sorted(baseline_rows, key=lambda r: int(r["seed"])),
    "set_side_run_count": 14,
    "design_note": (
        "A7 runs only missing set_dense_exact empty_only rows and joins them "
        "against existing matched A2 dense token baseline rows by D/d_ff/L/lr/seed."
    ),
}, indent=2) + "\n")
PY
}
...
echo "A7 empty_only calibration workers launched: GPU0 PID=${PID0}, GPU1 PID=${PID1}"
wait "${PID0}"
wait "${PID1}"
echo "=== A7 empty_only calibration sweep complete ==="

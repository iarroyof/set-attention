#!/usr/bin/env bash
# MRP-lca-cmp L4096 TRAJECTORY probe (generalization diagnostic 2c.1,
# pre-registered in audit/LCA_research_story_20260727.md Part IV 2c):
# b75 full routing + token control at L=4096, prefix supervision,
# all_past + score_mode=dense, native B4, 8000 updates with periodic
# validation every EVAL_EVERY updates (training.eval_every, runner
# support added in 9ab8104). Goal: locate the validation peak that the
# endpoint-only Stage B runs could not observe (0.8382 @4000 ->
# 0.7570 @8000). Token trajectory is the no-overfit control.
# Lizmark-only (token 33.7GB, b75 24.9GB — neither fits Blue).
# Labels l4096tj_*; never pooled with matrix/pilot/budget/stageb rows.
set -uo pipefail
...
GRID_ROOT="out/lca_cmp/l4096trajectory"
DONE_ROOT="${GRID_ROOT}/markers"
LOG_ROOT="logs/lca_cmp/${HOST_TAG}"
RESULT_TSV="${GRID_ROOT}/l4096trajectory_${HOST_TAG}.tsv"
...
run_row () { # label family topk seed gpu
  ...
  echo "=== RUN $(date '+%F %T') gpu${gpu} $name (updates=${MAX_UPDATES}, eval_every=${EVAL_EVERY}) ==="
  if [ "$DRY_RUN" = 1 ]; then
    echo "PLAN  $name -> $csv"
...
echo "=== LCA L4096TRAJECTORY ${HOST_TAG}: ROWS='${ROWS}', MAX_UPDATES=${MAX_UPDATES}, EVAL_EVERY=${EVAL_EVERY}, DRY_RUN=${DRY_RUN} ==="
set -- $ROWS
while [ $# -gt 0 ]; do
  spec1="$1"; shift
  spec2="${1:-}"; [ $# -gt 0 ] && shift
  ...
  run_row "l4096tj_${name1}" "$fam1" "$topk1" "$seed1" "$GPU0" &
  p0=$!
  if [ -n "$spec2" ]; then
    name2="${spec2%%:*}"; seed2="${spec2##*:}"
    read -r fam2 topk2 <<< "$(row_spec "$name2")"
    run_row "l4096tj_${name2}" "$fam2" "$topk2" "$seed2" "$GPU1" &
    p1=$!
    wait "$p0" "$p1"
  else
    wait "$p0"
...
echo "=== LCA L4096TRAJECTORY ${HOST_TAG} pass complete ==="

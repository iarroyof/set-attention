...
#
# ROLE=short: L=512, batch=16, exact backend.
# ROLE=long:  L=8192, batch=1, landmark backend with coverage 0.25.
...
case "${ROLE}" in
  short)
    ...
    COVERAGE_ARG=()
    DEFAULT_LOG_ROOT="logs/sd9_multiresolution_short"
    DEFAULT_OUT_ROOT="out/paper_mechanisms/sd9_multiresolution_short"
    ;;
  long)
    ...
    DEFAULT_LOG_ROOT="logs/sd9_multiresolution_long"
    DEFAULT_OUT_ROOT="out/paper_mechanisms/sd9_multiresolution_long"
    ;;
  *)
    echo "ROLE must be short or long" >&2
    exit 2
    ;;
esac
...
STATUS_PATH="${OUT_ROOT}/sd9_multiresolution_${ROLE}_status.tsv"
...
Path(f"audit/SD_9_multiresolution_prelaunch{suffix}.json").write_text(
...
run_one () {
  ...
  local lr_tag="${LR//./p}"
  local group="sd9_multiresolution_${ROLE}_${variant}"
  local name="sd9_${ROLE}_${variant}_blur${blur_pct}_L${SEQ_LEN}_B${BATCH}_${BACKEND}_seed${seed}"
...
worker_for_gpu () {
  ...
  for spec in "${ROW_SPECS[@]}"; do
    ...
    if [[ "${ROLE}" == "long" && "${variant}" == "mixed" ]]; then
      fine_heads=3
      coarse_heads=5
    fi
...
worker_for_gpu 0 0 > "${LOG_ROOT}/worker_gpu0.log" 2>&1 &
PID0="$!"
worker_for_gpu 1 1 > "${LOG_ROOT}/worker_gpu1.log" 2>&1 &
PID1="$!"

echo "${PID0}" > "${LOG_ROOT}/worker_gpu0.pid"
echo "${PID1}" > "${LOG_ROOT}/worker_gpu1.pid"
echo "SD-9 ${ROLE} workers launched: GPU0 PID=${PID0}, GPU1 PID=${PID1}"
echo "Status path: ${STATUS_PATH}"
wait "${PID0}"
wait "${PID1}"
echo "=== SD-9 ${ROLE} complete ==="

#!/usr/bin/env bash
# MRP-lca-cmp PREFIX-SUPERVISION 3-SEED mini calibration (user-approved
# 2026-07-20 after the combined probe: all_past+dense+topk1023+prefix b25
# reached val_acc 0.936 > token prefix 0.903 at L1024/seed0).
#
...
#   prefix3_token          token dense, data.supervision=prefix
#   prefix3_b25_fulltopk   b25, all_past, score_mode=dense, topk=1023, prefix
#   prefix3_b25_topk16     b25, all_past, score_mode=dense, topk=16, prefix
#
...
GRID_ROOT="out/lca_cmp/prefix3"
DONE_ROOT="${GRID_ROOT}/markers"
LOG_ROOT="logs/lca_cmp/${HOST_TAG}"
RESULT_TSV="${GRID_ROOT}/prefix3_${HOST_TAG}.tsv"
...
run_row () { # label family seed gpu ; family in token|fulltopk|topk16
  local label="$1" family="$2" seed="$3" gpu="$4"
  local name="${label}_L1024_seed${seed}"
  ...
  if [ "$family" != token ]; then
    ...
    if [ "$family" = fulltopk ]; then ov+=("model.router_topk=1023"); else ov+=("model.router_topk=16"); fi
  fi
...
worker () { # gpu labels...
  ...
  for fam in "$@"; do
    for seed in $SEEDS; do
      label="prefix3_${fam}"
      run_row "$label" "$fam" "$seed" "$gpu"
    done
  done
}

echo "=== LCA PREFIX3 blue: 9 rows, MAX_UPDATES=${MAX_UPDATES}, SEEDS='${SEEDS}', DRY_RUN=${DRY_RUN} ==="
worker "$GPU0" fulltopk topk16 &
w0=$!
worker "$GPU1" token &
w1=$!
wait "$w0" "$w1"
echo "=== LCA PREFIX3 ${HOST_TAG} pass complete ==="

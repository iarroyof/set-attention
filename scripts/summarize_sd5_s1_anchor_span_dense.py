#!/usr/bin/env python3
"""Validate and summarize SD-5 S1 anchor_span dense runs."""
...
def mode_spec(mode: str) -> ModeSpec:
    if mode == "smoke":
        return ModeSpec(
            mode=mode,
            raw=ROOT / "out" / "paper_mechanisms" / "sd5_s1_anchor_span_dense_smoke",
            logs=ROOT / "logs" / "sd5_s1_anchor_span_dense_smoke",
            config="configs/set_dictionary/s1_anchor_span_dense_smoke.yaml",
            group="sd5_s1_anchor_span_dense_smoke_D64_FF128",
            table=TABLES / "sd5_s1_anchor_span_dense_smoke_runs.tsv",
            manifest=CHECKS / "sd5_s1_anchor_span_dense_smoke_manifest.json",
            audit=AUDIT / "SD_5_s1_anchor_span_dense_smoke.md",
            seq_len=64,
            d_model=64,
            d_ff=128,
            layers=1,
    ...
    return ModeSpec(
        mode=mode,
        raw=ROOT / "out" / "paper_mechanisms" / "sd5_s1_anchor_span_dense",
        logs=ROOT / "logs" / "sd5_s1_anchor_span_dense",
        config="configs/set_dictionary/s1_anchor_span_dense.yaml",
        group="sd5_s1_anchor_span_dense_D384_FF1536",
        table=TABLES / "sd5_s1_anchor_span_dense_runs.tsv",
        manifest=CHECKS / "sd5_s1_anchor_span_dense_manifest.json",
        audit=AUDIT / "SD_5_s1_anchor_span_dense.md",
        seq_len=512,
        d_model=384,
        d_ff=1536,
        layers=6,
...
def validate_run(spec: ExpectedRun) -> dict[str, str]:
    ...
    if last.get("train/anchor_loss") not in {"", "NA", None}:
        raise ValueError(f"{spec.csv_path} logged anchor loss for S1 anchor-disabled run")

    return {
        "phase": "SD-5",
        "step": "S1",
        "mode": spec.spec.mode,
        "family_slug": "sd5_s1_anchor_span_dense",
        "family": "Set-Dictionary S1 anchor_span dense",

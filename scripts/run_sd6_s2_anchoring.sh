#!/usr/bin/env bash
# SD-6 S2 anchoring rescue: anchor_span plus causal pre-encoder anchor loss.
set -euo pipefail
...
if [[ "${SMOKE}" == "1" ]]; then
  ...
  LOG_ROOT="${LOG_ROOT:-logs/sd6_s2_anchoring_smoke}"
  OUT_ROOT="${OUT_ROOT:-out/paper_mechanisms/sd6_s2_anchoring_smoke}"
  GROUP_PREFIX="${GROUP_PREFIX:-sd6_s2_anchoring_smoke}"
  SEQ_LEN=64
  BATCH=2
  EPOCHS=1
  D_MODEL=64
  ...
  LOG_ROOT="${LOG_ROOT:-logs/sd6_s2_anchoring}"
  OUT_ROOT="${OUT_ROOT:-out/paper_mechanisms/sd6_s2_anchoring}"
  GROUP_PREFIX="${GROUP_PREFIX:-sd6_s2_anchoring}"
  SEQ_LEN=512
  BATCH=16
  EPOCHS=10
  D_MODEL=384
...
payload = {
    ...
    "contract": {
        ...
        "anchor.lambda_h": 0.1,
        "objective": "CE + lambda_h * anchor loss",
        "backend": "exact",
...
Path(f"audit/SD_6_s2_anchoring_prelaunch{suffix}.json").write_text(
...
if [[ "${SMOKE}" == "1" ]]; then
  ...
  PID1="$!"
  echo "SD-6 S2 anchoring workers launched: GPU0 PID=${PID0}, GPU1 PID=${PID1}"
  wait "${PID0}"
  wait "${PID1}"
fi
echo "=== SD-6 S2 anchoring complete ==="

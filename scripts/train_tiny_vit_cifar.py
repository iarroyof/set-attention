...
class TinyViT(nn.Module):
    def __init__(self, num_classes=10, dim=128, depth=4, heads=4, patch=4, img_size=32):
        super().__init__()
...
def main():
    ...
    if torchvision is None:
        raise RuntimeError("torchvision is required for CIFAR10 demo. Please install torchvision.")
    ...
    for ep in range(1, args.epochs + 1):
        ...
        for xb, yb in loader:
            ...
            n += xb.size(0)
            if n >= 4096:  # short epoch for demo
                break
        elapsed = time.time() - t0
        msg = f"[ViT-CIFAR][{args.attn}] epoch {ep:02d} loss {total/max(1,n):.4f}"
        if args.profile:

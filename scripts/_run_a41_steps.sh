#!/usr/bin/env bash
# A4.1 post-run steps: scp summarizer to blue-demon, run it, sync artifacts back.
# Run this AFTER run_a41_smoke.sh has completed on blue-demon.
set -euo pipefail
...
$SSHPASS scp scripts/summarize_a41_smoke.py \
    "$REMOTE:$REMOTE_REPO/scripts/summarize_a41_smoke.py"
echo "SCP OK"
...
python3 -c "
import json, sys
m = json.load(open('$REPO/out/paper_integrated_evidence/checks/a41_smoke_manifest.json'))

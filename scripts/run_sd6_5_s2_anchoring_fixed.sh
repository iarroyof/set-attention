#!/usr/bin/env bash
# SD-6.5 fixed S2 anchoring rescue: anchor_span plus trained causal pre-encoder.
set -euo pipefail
...
if [[ "${SMOKE}" == "1" ]]; then
  ...
  LOG_ROOT="${LOG_ROOT:-logs/sd6_5_s2_anchoring_fixed_smoke}"
  OUT_ROOT="${OUT_ROOT:-out/paper_mechanisms/sd6_5_s2_anchoring_fixed_smoke}"
  GROUP_PREFIX="${GROUP_PREFIX:-sd6_5_s2_anchoring_fixed_smoke}"
  SEQ_LEN=64
  BATCH=2
  EPOCHS=1
  D_MODEL=64
  ...
  LOG_ROOT="${LOG_ROOT:-logs/sd6_5_s2_anchoring_fixed}"
  OUT_ROOT="${OUT_ROOT:-out/paper_mechanisms/sd6_5_s2_anchoring_fixed}"
  GROUP_PREFIX="${GROUP_PREFIX:-sd6_5_s2_anchoring_fixed}"
  SEQ_LEN=512
  BATCH=16
  EPOCHS=10
  D_MODEL=384
...
payload = {
    ...
    "contract": {
        ...
        "anchor.enabled": True,
        "anchor.target": "pre_encoder",
        "anchor.pre_encoder_layers": 2,
        "anchor.lambda_h": 0.1,
        "anchor.lambda_pre": 1.0,
        "anchor.pre_encoder_head": True,
        "objective": "CE + lambda_pre * pre-encoder CE + lambda_h * anchor loss",
        "backend": "exact",
...
Path(f"audit/SD_6_5_s2_anchoring_fixed_prelaunch{suffix}.json").write_text(
...
if [[ "${SMOKE}" == "1" ]]; then
  ...
  PID1="$!"
  echo "SD-6.5 fixed S2 anchoring workers launched: GPU0 PID=${PID0}, GPU1 PID=${PID1}"
  wait "${PID0}"
  wait "${PID1}"
fi
echo "=== SD-6.5 fixed S2 anchoring complete ==="

#!/usr/bin/env bash
# MRP-lca-cmp L2048 PILOT (user-approved 2026-07-24,
# docs/agent_plans/mrp_lca_cmp.md "Approved Diagnostic Probes" staging 2).
#
# Early L2048 smoke (seed 0 first): does the b75 L1024 frontier result
# transfer, and does full routing keep the VRAM advantage when dense
# all-past scores are O(L^2)?
# Rows (L=2048, seed 0, native B4, max_updates=2000, prefix supervision):
#   l2048pilot_token        token dense
#   l2048pilot_b75full      b75 all_past dense topk=2047 (full)
#   l2048pilot_b75topk256   b75 all_past dense topk=256 (likely sparse point)
# Diagnostic labels, never pooled with matrix rows.
# Resumable; strict log scan before .done.
set -uo pipefail
...
GRID_ROOT="out/lca_cmp/l2048pilot"
DONE_ROOT="${GRID_ROOT}/markers"
LOG_ROOT="logs/lca_cmp/${HOST_TAG}"
RESULT_TSV="${GRID_ROOT}/l2048pilot_${HOST_TAG}.tsv"
...
echo "=== LCA L2048PILOT ${HOST_TAG}: SEEDS='${SEEDS}', MAX_UPDATES=${MAX_UPDATES}, DRY_RUN=${DRY_RUN} ==="
for seed in $SEEDS; do
  run_row l2048pilot_b75full b75 2047 "$seed" "$GPU0" &
  p0=$!
  run_row l2048pilot_token token 0 "$seed" "$GPU1" &
  p1=$!
  wait "$p0" "$p1"
  run_row l2048pilot_b75topk256 b75 256 "$seed" "$GPU0"
done
echo "=== LCA L2048PILOT ${HOST_TAG} pass complete ==="

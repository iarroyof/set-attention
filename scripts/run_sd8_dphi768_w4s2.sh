#!/usr/bin/env bash
# SD-8 follow-up: (4,2) all_past CE-only with doubled dictionary atom width.
set -euo pipefail
...
if [[ "${SMOKE}" == "1" ]]; then
  ...
  LOG_ROOT="${LOG_ROOT:-logs/sd8_all_past_dense_dphi768_w4s2_smoke}"
  OUT_ROOT="${OUT_ROOT:-out/paper_mechanisms/sd8_all_past_dense_dphi768_w4s2_smoke}"
  GROUP_PREFIX="${GROUP_PREFIX:-sd8_all_past_dense_dphi768_w4s2_smoke}"
  SEQ_LEN=64
  BATCH=2
  EPOCHS=1
  D_MODEL=64
  ...
  LOG_ROOT="${LOG_ROOT:-logs/sd8_all_past_dense_dphi768_w4s2}"
  OUT_ROOT="${OUT_ROOT:-out/paper_mechanisms/sd8_all_past_dense_dphi768_w4s2}"
  GROUP_PREFIX="${GROUP_PREFIX:-sd8_all_past_dense_dphi768_w4s2}"
  SEQ_LEN=512
  BATCH=16
  EPOCHS=10
  D_MODEL=384
...
record_prelaunch () {
  python3 - "$SMOKE" "$CONFIG" "$OUT_ROOT" "$LOG_ROOT" "$D_PHI" "$SET_STATE_DIM" <<'PY'
import json
import subprocess
import sys
from pathlib import Path
...
out_root = sys.argv[3]
log_root = sys.argv[4]
d_phi = int(sys.argv[5])
set_state_dim = int(sys.argv[6])
...
payload = {
    ...
    "topologies": [{"w": 4, "s": 2, "M": 255}],
    "reference_policy": "Compare against existing baselines and SD-8; do not rerun refs.",
}
suffix = "_smoke" if smoke else ""
Path(f"audit/SD_8_1_dphi768_w4s2_prelaunch{suffix}.json").write_text(
...
run_one () {
  ...
  local LR_TAG="${LR//./p}"
  local GROUP="${GROUP_PREFIX}_D${D_MODEL}_FF${D_FF}_dphi${D_PHI}_setdim${SET_STATE_DIM}"
  local NAME="${GROUP}_L${SEQ_LEN}_w${WINDOW}_s${STRIDE}_M${M}_lr${LR_TAG}_seed${SEED}"
...
if [[ "${SMOKE}" == "1" ]]; then
  ...
  PID1="$!"
  echo "SD-8.1 dphi768 w4s2 workers launched: GPU0 PID=${PID0}, GPU1 PID=${PID1}"
  wait "${PID0}"
  wait "${PID1}"
fi
echo "=== SD-8.1 dphi768 w4s2 complete ==="

#!/usr/bin/env bash
# A3.2 post-patch steps: scp script to blue-demon, run summarizer, sync artifacts back.
set -euo pipefail
...
$SSHPASS scp scripts/summarize_a3_pooltau_sweep.py \
    "$REMOTE:$REMOTE_REPO/scripts/summarize_a3_pooltau_sweep.py"
echo "SCP OK"
...
for f in \
    ...
    "audit/A3_2_pooltau_sweep.md"; do
...
python3 -c "
import json, sys
m = json.load(open('$REPO/out/paper_integrated_evidence/checks/a3_pooltau_sweep_manifest.json'))

    ...
    with tempfile.TemporaryFile(mode="w+") as stderr_file:
        ...
        while True:
            ...
            if run_timeout_s > 0 and (time.monotonic() - start) >= run_timeout_s:
                print(f"[sweep] warn: timeout after {run_timeout_s:.0f}s; terminating pid {proc.pid}")
                try:
...
def main():
    ...
    ap.add_argument(
        "--tasks",
        nargs="+",
        choices=["lm", "seq2seq", "textdiff", "vit"],
        default=["lm", "textdiff"],
        help="Subset of tasks to run (default: lm textdiff).",
    )
    ...
    ap.add_argument("--sweep-yaml", type=str, default="", help="W&B-style sweep yaml to expand into multiple Stage B runs.")
    ...
    if args.sweep_yaml:
        ...
        if has_model_type:
            if any(arg == "--model-type" or arg.startswith("--model-type=") for arg in base_args):
                print("[sweep] note: --model-type ignored when sweep YAML defines model-type.")
        ...
        for idx, combo in enumerate(combos, start=1):
            ...
            print(f"[sweep] sweep {idx}/{len(combos)}: {combo}")
    ...
    if args.model_type == "ska" and args.cache_mode == "full" and not args.precache:
        ...
        if "seq2seq" in tasks and args.seq_lengths:
        ...
        if "textdiff" in tasks and args.textdiff_lengths:
